...
class PGExplainer(nn.Module):
    ...
    def explain_node(
        ...
        batched_inverse_indices = batched_graph.ndata["train"].nonzero().squeeze(1)
...
class HeteroPGExplainer(PGExplainer):
    ...
    def train_step_node(self, nodes, graph, feat, temperature, **kwargs):
        ...
        loss = self.loss(
            torch.cat([prob[ntype][nid] for ntype, nid in inverse_indices.items()]),
            torch.cat([pred[ntype][nid] for ntype, nid in inverse_indices.items()]),
        )
        return loss
    ...
    def explain_node(
        ...
        for target_ntype, target_nids in nodes.items():
            ...
            for target_nid in target_nids:
                ...
                self_emb = homo_sg_embed[inverse_indices[target_ntype][0]].repeat(
                    sg.num_edges(), 1
                )
                emb = torch.cat([col_emb, row_emb, self_emb], dim=-1)
                batched_embed.append(emb)
        ...
        probs = {ntype: F.softmax(logits[ntype], dim=-1) for ntype in logits.keys()}

        batched_inverse_indices = {
            ntype: batched_hetero_graph.nodes[ntype].data["train"].nonzero().squeeze(1)
            for ntype in batched_hetero_graph.ntypes
        }
